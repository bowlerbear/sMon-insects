...
#Analysis of the Odonata data -juvenile data
...
write.table(mainSpeciesDF,file=paste0("speciesTaskID_juv_",state,".txt"),sep="\t",row.names=FALSE)
...
write.table(data.frame(Species=mainSpecies),file=paste0("mainSpecies_juvs_",state,".txt"),sep="\t") 
...
```{r}
#get annual indicies
load(paste0("model-outputs/out_nuRecords_juveniles_",state,".RData"))
out2 <- combineModels(out)
outTrends <- getBUGSfits(out2,param="psi.fs") 

save(outTrends,file=paste0("outTrends_nuRecords_juv_",state,".RData"))
