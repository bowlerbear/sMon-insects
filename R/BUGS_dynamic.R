cat("
  ...
  #first year occupancy, model
  for (i in 1:nsite){ 
    z[i,1] ~ dbern(muZ[i,1])
    logit(muZ[i,1]) <- int + site.variation
  }
  ...
  for (i in 1:nsite){ 
    for (t in 2:nyear){
      z[i,t] ~ dbern(muZ[i,t]) 
      muZ[i,t] <- persist[i,t-1]*muZ[i,t-1] + colonization[i,t-1]*(1-muZ[i,t-1])
      logit(persist) <- int + dynamic.variation.bysite
      logit(colonization) <- int + dynamic variation.bysite
    }
  }   
  ...
  # State model priors
    ...
    #trend priors
    #trend.int ~ dnorm(0,0.01)
    #trend.year ~ dnorm(0,0.01)
