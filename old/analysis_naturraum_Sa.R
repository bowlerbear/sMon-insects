...
#the models were using on the HPC
#using the script
#analysis_HPC_nation_naturraum_Sa.R script
...
speciesTaskID <- read.delim(paste0("/data/idiv_ess/Odonata/speciesTaskID_adult_Sa.txt"),as.is=T)
#get task id
...
#read in naturraum data
naturraum <- readRDS("naturraum_Saarland.rds")
nrow(subset(naturraum,is.na(nat_reg_id)))
naturraum <- subset(naturraum, !is.na(nat_reg_id))

#plot data
library(ggplot2)
qplot(lon,lat,data=adultData)
...
#subset to those for which we have naturrum data for
adultData <- subset(adultData, MTB_Q %in% naturraum$MTB_Q)
...
#add natur raum
listlengthDF$Naturraum <- naturraum$nat_reg_id[match(listlengthDF$MTB_Q,naturraum$MTB_Q)]
listlengthDF$nnIndex <- as.numeric(factor(paste0(listlengthDF$Naturraum)))
...
#######################################################################################
#how many species and records are there in each naturraum?
adultData$Naturraum <- naturraum$nat_reg_id[match(adultData$MTB_Q,naturraum$MTB_Q)]

ddply(adultData,.(Naturraum),summarise,
...
siteInfo <- unique(listlengthDF[,c("stateIndex","mtbIndex","siteIndex","boxIndex","nnIndex","Naturraum","State")])
head(siteInfo)

table(siteInfo$State)#most are from Niedersachsen
table(siteInfo$nnIndex)
table(listlengthDF$nnIndex)

naturraumTranslation <- unique(siteInfo[,c("nnIndex","Naturraum")])
subset(naturraumTranslation,State=="Niedersachsen")
...
modelfile="/R/BUGS_dynamic_nation_naturraum.txt"
...
#read in files from the HPC
mdir <- "C:/Users/db40fysa/Nextcloud/sMon-Analyses/Git/sMon-insects/model-outputs/Odonata_adult_dynamic_NN_Sa/5684368"
#the folder 5684368 contains output of the original (non-dynamic sparta output)

myfiles <- list.files(mdir)
#load each one and combine
modelSummary <- ldply(myfiles,function(x){
  modelSummary <- readRDS(paste(mdir,x,sep="/"))
  modelSummary  <- data.frame(modelSummary )
  modelSummary$Param <- row.names(modelSummary )
  modelSummary$File <- x
  return(modelSummary)
})

table(modelSummary$Rhat<1.1)

#get species
modelSummary$Species <- gsub("outSummary_dynamic_nation_state_adult_","",modelSummary$File)
modelSummary$Species <- gsub(".rds","",modelSummary$Species)

#we want the psi.raums
modelSummary  <- modelSummary [grepl("psi.raum",modelSummary $Param),]
...
modelSummary$nnIndex <- sapply(modelSummary$Temp,function(x)strsplit(x,",")[[1]][1])
unique(modelSummary$nnIndex)
modelSummary$Year <- modelSummary$Year + 1979

#match with state and naturruam
modelSummary <- merge(modelSummary,naturraumTranslation,by="nnIndex")

#restrict to Niedersaschen
#modelSummary <- subset(modelSummary, State=="Niedersachsen")
#unique(modelSummary$Naturraum)

#unique(modelSummary$mean)

#plot time series for each natuuraum
library(ggplot2)
library(boot)
ggplot(subset(modelSummary,Naturraum==2))+
  geom_line(aes(x=Year,y=mean))+
  geom_ribbon(aes(x=Year,ymin=X2.5.,ymax=X97.5.),alpha=0.5)+
  facet_wrap(~Species)+
  ggtitle("Natur reg id 2")

#dont plot the CIs, colour code the naturerg
ggplot(modelSummary,aes(x=Year,y=mean,group=factor(Naturraum),colour=factor(Naturraum)))+
  geom_line(aes(x=jitter(Year,10),y=jitter(mean,10),group=factor(Naturraum),colour=factor(Naturraum)))+
  facet_wrap(~Species)

ggplot(subset(modelSummary,Species=="Aeshna grandis"))+
  geom_line(aes(x=Year,y=mean))+
  geom_ribbon(aes(x=Year,ymin=X2.5.,ymax=X97.5.),alpha=0.5)+
