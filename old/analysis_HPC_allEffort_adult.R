...
getListLength<-function(df){
  ...
  out <- ddply(df,.(visit,Date,Year,month,day,MTB_Q),summarise,
               ...
               nuAnzahl=sum(Anzahl_min,na.rm=T),
               Richness2=mean(Richness),
               RpS = nuRecords/nuSpecies,
...
bugs.data <- list(nsite = length(unique(listlengthDF$siteIndex)),
                    ...
                    nuSS = log(listlengthDF$samplingSites) - log(median(listlengthDF$samplingSites)),
                    expertise = log(listlengthDF$expertise)-median(log(listlengthDF$expertise)),
                    nuAnzahl = log(listlengthDF$nuAnzahl)-median(log(listlengthDF$nuAnzahl)),
...
bugs.data$Effort_v2 <- bugs.data[[effort2]]
  
#specify parameters to monitor
params <- c("phenol.p","phenol2.p","effort.p","effort2.p","ss.p","expert.p","anzahl.p","psi.fs")
