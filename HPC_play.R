...
#speciesTaskID <- read.delim("speciesTaskID_adult.txt",as.is=T)
#get task id
#get species for this task
#myspecies <- speciesTaskID$Species[which(speciesTaskID$TaskID==1)] 
...
out <- jags(bugs.data, inits=inits, params, modelfile, n.thin=nt,
            n.chains=n.cores, n.burnin=250,n.iter=1000,parallel=T)

Sys.time()

#on HPC, 1000 iteractions took 105 mins
#we can do about 13 of these per day, i.e., 13,000...too slow still :(
########################################################################################