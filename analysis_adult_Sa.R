...
df <- readRDS(paste0("derived-data/",stage,"_datafile_",state,".rds"))

#load the relational table of task ids and species
speciesTaskID <- read.delim(paste0("speciesTaskID_",stage,"_",state,".txt"),as.is=T)
#species are labelled 1:length(species)

#get species for this task
myspecies <- speciesTaskID$Species[1] 
...
yearDF <- read.delim(paste0("yearDF_",stage,"_",state,".txt"),as.is=T)
df <- subset(df, Year>=(min(yearDF$Year)) & Year<2017)

#get phenology info
speciesDays <- read.delim(paste0("speciesDays_",stage,"_",state,".txt"),as.is=T)
...
out <- jags(bugs.data, inits=inits, params, modelfile, n.thin=nt,
               n.chains=3, n.burnin=20000,n.iter=100000,parallel=T)
